...
class FakeDocumentRepository {
	async findById() {
		return { id: "doc_1", ownerUserId: "user_1", flow: "master", currentPhase: "profile", readiness: { ready: false, missing: [] } };
	}
}

class FakeFieldStateRepository {
	async list() {
		return [{ flow: "master", phaseKey: "profile", fieldKey: "organization_name", status: "confirmed", value: "PT Maju Jaya" }];
	}
}

class FakeConversationRepository {
	async recentMessages() {
		return [{ id: "m1", role: "assistant", content: "Halo", createdAt: "2026-05-15T00:00:00.000Z", metadata: {} }];
	}
}
...
describe("interview read routes", () => {
	it("returns readiness checklist", async () => {
		...
		app.route("/", createInterviewReadRoutes({ documents: new FakeDocumentRepository() as any, fieldStates: new FakeFieldStateRepository() as any, conversations: new FakeConversationRepository() as any }));
