...
import {
  trainerBrainstormingOptionalFields,
  trainerBrainstormingRequiredFields,
  trainerSkkniRequiredFields,
  trainerTrainingDetailsGeneratedFields,
} from './trainer-fields'
...
export function buildTrainerReadiness(states: FieldStateSnapshot[]): ReadinessResult {
  ...
  const requireField = (phaseKey: string, fieldKey: string) => {
    ...
    if (!state || !['captured', 'confirmed'].includes(state.status) || !hasUsableValue(state.value))
      missing.push(key)
  }

  for (const fieldKey of trainerBrainstormingRequiredFields) requireField('brainstorming', fieldKey)
  for (const requirement of trainerSkkniRequiredFields)
  ...
  for (const fieldKey of trainerTrainingDetailsGeneratedFields) {
    ...
    if (!state || !hasUsableValue(state.value))
      optionalGaps.push(keyOf('training_details', fieldKey))
  }
