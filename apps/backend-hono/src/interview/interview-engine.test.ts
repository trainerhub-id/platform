    ...
    this.states = this.states.filter(
      (state) =>
        !(
          ...
          state.phaseKey === snapshot.phaseKey &&
          state.fieldKey === snapshot.fieldKey
        ),
    )
...
describe('InterviewEngine', () => {
  it('saves user message, field patch, updates state, and returns stream response', async () => {
    ...
    const response = await engine.handleMessage({
      documentId: 'doc_1',
      userId: 'user_1',
      message: 'Lembaganya PT Maju Jaya',
    })
  ...
  it('confirms pending suggestions and compiles confirmed value into master_json', async () => {
      ...
      extraction: {
        async extract() {
          return {
            ...
            pendingSuggestions: [],
            confirmedPendingFields: [
              { flow: 'master', phaseKey: 'profile', fieldKey: 'program_name' },
            ],
            generateConfirmed: false,
          }
        },
    ...
    const state = fieldStates.states.find(
      (item) => item.phaseKey === 'profile' && item.fieldKey === 'program_name',
    )
    ...
    expect(documents.updated.masterJson.brainstorming_master.program_name).toBe(
  ...
  it('preserves existing confirmed values from user_explicit patches', async () => {
    ...
    await engine.handleMessage({
      documentId: 'doc_1',
      userId: 'user_1',
      message: 'Lembaganya PT Draft',
    })

    const state = fieldStates.states.find(
      (item) => item.phaseKey === 'profile' && item.fieldKey === 'organization_name',
    )
  ...
  it('handles trainer field patches with trainer compiler and readiness', async () => {
    ...
    const response = await engine.handleMessage({
      documentId: 'doc_1',
      userId: 'user_1',
      message: 'Trainernya Budi, bidang Digital Marketing',
    })
  ...
  it('fills Trainer generated training details after core facts are captured', async () => {
    ...
    const engine = new InterviewEngine({
          ...
          return {
            patches: [
              ...
              },
              {
                ...
                phaseKey: 'brainstorming',
                fieldKey: 'audience',
                value: 'UMKM yang mau naik kelas',
                source: 'user_explicit',
                confidence: 0.95,
              },
              {
                ...
                phaseKey: 'brainstorming',
                fieldKey: 'outcome',
                value: 'UMKM bisa membuat strategi pemasaran sendiri',
                source: 'user_explicit',
                confidence: 0.95,
              },
            ],
      ...
      response: {
        async stream() {
          return new Response('Data inti cukup. Saya bisa carikan unit SKKNI.')
        },
      },
    })
    ...
    await engine.handleMessage({
      documentId: 'doc_1',
      userId: 'user_1',
      message: 'Saya Ujang, mandiri, bidang marketing untuk UMKM.',
    })

    expect(documents.updated.masterJson.training_details.program_name).toBe(
      'Pelatihan Marketing untuk UMKM yang mau naik kelas',
    )
  ...
  it('fills Master generated program_name from core facts', async () => {
          ...
          return {
            patches: [
              ...
              },
              {
                flow: 'master',
                phaseKey: 'profile',
                fieldKey: 'industry_problem',
                value: 'Iklan boros tapi hasil minim',
                source: 'user_explicit',
                confidence: 0.95,
              },
              {
                flow: 'master',
                phaseKey: 'profile',
                fieldKey: 'program_goal',
                value: 'Meningkatkan penjualan UMKM melalui strategi digital',
                source: 'user_explicit',
                confidence: 0.95,
              },
              {
    ...
    await engine.handleMessage({
      documentId: 'doc_1',
      userId: 'user_1',
      message: 'Program digital marketing untuk owner UMKM di Bekasi.',
    })

    expect(documents.updated.masterJson.brainstorming_master.program_name).toBe(
      'Pelatihan Digital Marketing untuk Owner UMKM',
    )
  ...
  it('passes Trainer unit search confirmation to the response service', async () => {
    ...
    fieldStates.states = [
      ...
      },
      {
        ...
        status: 'captured',
        source: 'user_explicit',
        value: 'Membuat strategi pemasaran digital',
        pendingSuggestion: null,
      },
    ]
    const conversations = new FakeConversationRepository()
