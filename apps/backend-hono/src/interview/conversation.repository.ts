...
export class ConversationRepository {
  ...
  async addMessage(input: { documentId: string; role: ConversationRole; content: string; metadata?: unknown }) {
    const [message] = await this.client
      .insert(conversationMessages)
      .values({ documentId: input.documentId, role: input.role, content: input.content, metadata: input.metadata ?? {} })
      .returning();
    return message;
  }
