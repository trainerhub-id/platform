  ...
  async handleMessage(input: {
    ...
    if (doc.flow !== 'master' && doc.flow !== 'trainer') {
      return jsonErrorResponse(
        400,
        'FLOW_NOT_SUPPORTED',
        'Only master and trainer flows are supported',
      )
    }

    const responseInput = await this.transactionRunner.run(
      async ({ documents, fieldStates, conversations }) => {
        ...
        const phase =
          flow === 'master'
            ? getNextMasterPhase(existingStates)
            : getNextTrainerPhase(existingStates)
        ...
        const pendingConfirmations = new Set(
          extracted.confirmedPendingFields.map((field) => stateKey(field.phaseKey, field.fieldKey)),
        )
        ...
        for (const confirmed of extracted.confirmedPendingFields) {
          ...
          const existing = existingStates.find(
            (state) =>
              state.phaseKey === confirmed.phaseKey &&
              state.fieldKey === confirmed.fieldKey &&
        ...
        for (const patch of extracted.patches) {
          ...
          const existing = existingStates.find(
            (state) => state.phaseKey === patch.phaseKey && state.fieldKey === patch.fieldKey,
          )
        ...
        for (const generated of buildGeneratedFieldPatches(
          flow,
          statesAfterExtraction as FieldStateSnapshot[],
        )) {
          await fieldStates.upsert({
            documentId: input.documentId,
            flow: generated.flow,
        ...
        const readiness =
          flow === 'master'
            ? buildMasterReadiness(stateSnapshot)
            : buildTrainerReadiness(stateSnapshot)
        const nextPhase =
          flow === 'master' ? getNextMasterPhase(stateSnapshot) : getNextTrainerPhase(stateSnapshot)
        const masterJson =
          flow === 'master' ? compileMasterJson(stateSnapshot) : compileTrainerJson(stateSnapshot)
        ...
        const capturedFields = stateSnapshot
          .filter(
            (s) =>
              ['captured', 'confirmed'].includes(s.status) &&
              s.value !== null &&
              s.value !== undefined,
          )
...
function persistAssistantMessageAfterStream(
  response: Response,
  save: (assistantText: string) => Promise<void>,
): Response {
  if (!response.body) {
    return response
  }
