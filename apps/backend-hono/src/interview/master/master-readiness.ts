...
import {
  masterProfileGeneratedFields,
  masterProfileOptionalFields,
  masterProfileRequiredFields,
  masterSkkniRequiredFields,
} from './master-fields'
...
export function buildMasterReadiness(states: FieldStateSnapshot[]): ReadinessResult {
  ...
  const requireField = (phaseKey: string, fieldKey: string) => {
    ...
    if (
      ...
      !hasUsableValue(state.value)
    ) {
      missing.push(key)
    }
  }

  for (const fieldKey of masterProfileRequiredFields) {
    requireField('profile', fieldKey)
  }
