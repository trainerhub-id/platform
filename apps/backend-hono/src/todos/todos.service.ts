...
export class TodosService {
	...
	async initializeTodosForUser(userId: string, batchId?: string) {
		...
		const definitions = Object.values(TODO_DEFINITIONS).filter((definition) => definition.role === "participant");
		...
		await this.repository.pruneUserTodosToKeys(profile.id, definitions.map((definition) => definition.key));
	...
	async initializeTodosForBatch(batchId?: string) {
		const definitions = Object.values(TODO_DEFINITIONS).filter((definition) => definition.role === "admin");
		return this.repository.insertMany(definitions.map((definition) => ({
			userId: null,
			batchId,
			key: definition.key,
			title: definition.title,
	...
	async syncTodosWithProfileState(userId: string) {
		...
		const hasPaidAccess = profile.paymentStatus === "paid" || await this.repository.hasPaidEnrollment(profile.id);
		await this.repository.setStatusByKey(profile.id, "lakukan_pembayaran", hasPaidAccess ? "done" : "todo");
		await this.repository.setStatusByKey(profile.id, "selesaikan_kelas", await this.repository.hasCompletedLesson(profile.id) ? "done" : "todo");
		await this.repository.setStatusByKey(profile.id, "upload_dokumen", await this.repository.hasUploadedDocument(profile.id) ? "done" : "todo");
		await this.repository.setStatusByKey(profile.id, "coba_ai", await this.repository.hasAiDocument(userId) ? "done" : "todo");
	}
}
