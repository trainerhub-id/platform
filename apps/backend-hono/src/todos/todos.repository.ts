...
import { documents, dokumenPeserta, peserta, pesertaBatch, pesertaCourseProgress, todos, users, type NewTodo } from "../db/schema";
...
export class TodosRepository {
	...
	async ensurePesertaForUser(userId: string) {
		...
		const [profileByEmail] = await db.select().from(peserta).where(eq(peserta.email, user.email)).limit(1);
		if (profileByEmail) {
			const [updated] = await db.update(peserta).set({ clerkId: user.id, updatedAt: new Date() }).where(eq(peserta.id, profileByEmail.id)).returning();
			return updated ?? profileByEmail;
		}

		const [created] = await db
			.insert(peserta)
			.values({ clerkId: user.id, nama: user.name || user.email, email: user.email, paymentStatus: "pending" })
			.returning();
		return created ?? null;
	}
	...
	async findAdmin(batchId?: string) {
		...
		return db.select().from(todos).where(and(...conditions)).orderBy(asc(todos.createdAt));
	}
	...
	async updateDefinitionMetadata(userId: string, values: NewTodo[]) {
		await Promise.all(values.map((value) => db
			.update(todos)
			.set({
				title: value.title,
	...
	async setStatusByKey(userId: string, key: string, status: "todo" | "in_progress" | "waiting_review" | "done") {
		const [row] = await db
			.update(todos)
			.set({ status, completedAt: status === "done" ? new Date() : null, updatedAt: new Date() })
			.where(and(eq(todos.userId, userId), eq(todos.key, key)))
	...
	async hasPaidEnrollment(pesertaId: string) {
		const [row] = await db.select().from(pesertaBatch).where(and(eq(pesertaBatch.pesertaId, pesertaId), eq(pesertaBatch.paymentStatus, "paid"))).limit(1);
		return !!row;
	}

	async hasUploadedDocument(pesertaId: string) {
		const [row] = await db.select({ id: dokumenPeserta.id }).from(dokumenPeserta).where(eq(dokumenPeserta.pesertaId, pesertaId)).limit(1);
		return !!row;
	}

	async hasCompletedLesson(pesertaId: string) {
		const [row] = await db.select({ id: pesertaCourseProgress.id }).from(pesertaCourseProgress).where(and(eq(pesertaCourseProgress.pesertaId, pesertaId), eq(pesertaCourseProgress.status, "selesai"))).limit(1);
		return !!row;
	}

	async hasAiDocument(userId: string) {
		const [row] = await db.select({ id: documents.id }).from(documents).where(and(eq(documents.ownerUserId, userId), inArray(documents.flow, ["master", "trainer"]), isNull(documents.deletedAt))).limit(1);
		return !!row;
	}
}
