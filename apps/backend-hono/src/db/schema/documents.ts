...
export const conversationMessages = pgTable(
  ...
  (table) => ({
    documentCreatedIdx: index("conversation_messages_document_created_idx").on(table.documentId, table.createdAt),
  }),
);
...
export const documentPayloadSnapshots = pgTable(
  ...
  (table) => ({
    ...
    documentTypeHashUnique: uniqueIndex("document_payload_snapshots_document_type_hash_unique_idx").on(
      table.documentId,
      table.documentType,
      table.payloadHash,
    ),
...
export const generationJobs = pgTable(
  ...
  (table) => ({
    jobRequestUnique: uniqueIndex("generation_jobs_job_request_unique_idx").on(table.jobType, table.requestKey),
    documentCreatedIdx: index("generation_jobs_document_created_idx").on(table.documentId, table.createdAt),
  }),
);
...
export const generatedFiles = pgTable(
  ...
  (table) => ({
    documentCreatedIdx: index("generated_files_document_created_idx").on(table.documentId, table.createdAt),
