import {
  ...
  type EnsurePendingEnrollmentInput,
  type SetPaymentStatusInput,
...
type EnrollmentRepositoryLike = {
  ...
  ensurePendingEnrollmentForPayment(
    input: EnsurePendingEnrollmentInput,
  ): Promise<{ id: string; pesertaId: string }>
}
...
export class EnrollmentService {
  ...
  async markPaid(enrollmentId: string) {
    return this.repository.setPaymentStatus({
      enrollmentId,
      paymentStatus: 'paid',
