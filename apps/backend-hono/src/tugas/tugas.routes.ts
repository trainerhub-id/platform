...
const reviewSchema = z.object({
  nilai: z.string().min(1).max(10),
  catatanTrainer: z.string().optional().nullable(),
})
type TugasVariables = AuthVariables & { requestId: string }
type UserLike = { id: string; role?: string }
type TugasServiceLike = Pick<
  TugasService,
  'list' | 'upload' | 'review' | 'getPesertaTugas' | 'getPesertaTugasById' | 'getUploads'
>

export function createTugasRoutes(service: TugasServiceLike = new TugasService()) {
  const app = new Hono<{ Variables: TugasVariables }>()

  app.get('/tugas/list', requireAuth, requireRole(['peserta', 'admin']), async (c) =>
  ...
  app.post('/tugas/upload', requireAuth, requireRole(['peserta']), async (c) => {
    ...
    if (typeof tugasId !== 'string' || !z.string().uuid().safeParse(tugasId).success)
      return errorResponse(c, 400, 'VALIDATION_ERROR', 'valid tugasId is required')
  ...
  app.patch('/tugas/:id/review', requireAuth, requireRole(['admin']), async (c) => {
    ...
    return c.json(
      await service.review(c.req.param('id'), {
        nilai: parsed.data.nilai,
        catatanTrainer: parsed.data.catatanTrainer ?? null,
      }),
    )
  })
  ...
  app.get('/tugas/peserta/:id', requireAuth, requireRole(['admin']), async (c) =>
    c.json(await service.getPesertaTugasById(c.req.param('id'))),
  )
  app.get(
    '/tugas/:tugasPesertaId/uploads',
    requireAuth,
    requireRole(['peserta', 'admin']),
    async (c) => c.json(await service.getUploads(c.req.param('tugasPesertaId'))),
  )
