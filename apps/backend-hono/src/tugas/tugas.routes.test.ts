...
describe("tugas routes", () => {
	test("returns task list for peserta", async () => {
		...
		app.route("/", createTugasRoutes({ list: async () => [{ id: "tugas_1", namaTugas: "Upload KTP" }], upload: async () => ({}), review: async () => ({}), getPesertaTugas: async () => [], getPesertaTugasById: async () => [], getUploads: async () => [] } as never));
