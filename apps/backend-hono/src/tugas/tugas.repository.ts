...
export class TugasRepository {
	...
	async createSubmission(pesertaId: string, tugasId: string) {
		const [row] = await db.insert(tugasPeserta).values({ pesertaId, tugasId, status: "submitted" }).returning();
		if (!row) throw new Error("TASK_SUBMISSION_NOT_FOUND");
		return row;
	}

	async markSubmitted(id: string) {
		const [row] = await db.update(tugasPeserta).set({ status: "submitted", updatedAt: new Date() }).where(eq(tugasPeserta.id, id)).returning();
		return row ?? null;
	}

	async createUpload(tugasPesertaId: string, fileUrl: string) {
		const [row] = await db.insert(tugasPesertaUpload).values({ tugasPesertaId, fileUrl }).returning();
	...
	async review(id: string, input: { nilai: string; catatanTrainer?: string | null }) {
		const [row] = await db
			.update(tugasPeserta)
			.set({ nilai: input.nilai, catatanTrainer: input.catatanTrainer ?? null, status: "reviewed", updatedAt: new Date() })
	...
	async listUploads(tugasPesertaId: string) {
		return db.select().from(tugasPesertaUpload).where(eq(tugasPesertaUpload.tugasPesertaId, tugasPesertaId));
	}
}
