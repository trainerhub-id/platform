...
export class ObjectStorageService {
  ...
  constructor(options: ObjectStorageOptions = {}) {
    ...
    this.sign =
      options.sign ??
      ((client, command, signOptions) =>
        getSignedUrl(client as never, command as never, signOptions))
  ...
  async uploadBuffer(
    ...
    if (!this.validateBufferSize(buffer))
      throw new Error(`Buffer size exceeds ${this.maxFileSizeMB}MB limit`)
  ...
  async upload(
    ...
    if (!this.validateBufferSize(file.buffer))
  ...
  async getSignedUrl(key: string, expiresIn = 3600): Promise<string> {
    return this.sign(
      this.client,
      new GetObjectCommand({ Bucket: this.bucket, Key: key }) as { input: { Key?: string } },
      { expiresIn },
    )
  }
