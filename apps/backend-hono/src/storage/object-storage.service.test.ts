...
describe('object storage helpers', () => {
  ...
  it('builds legacy-compatible certificate paths', () => {
    ...
    expect(buildCertificatePath('peserta_1', 'course_1')).toBe(
      'storage/peserta/peserta_1/certificates/course_1',
    )
  })
})
...
describe('ObjectStorageService', () => {
  it('uploads a buffer to S3-compatible storage and returns signed URL', async () => {
    ...
    const result = await storage.uploadBuffer(
      Buffer.from('pdf'),
      'storage/peserta/p1/certificates/c1',
      'application/pdf',
    )

    expect(result).toEqual({
      key: 'storage/peserta/p1/certificates/c1/123.pdf',
      url: 'signed:storage/peserta/p1/certificates/c1/123.pdf',
    })
  ...
  it('rejects buffers over max file size', async () => {
    ...
    await expect(
      storage.uploadBuffer(Buffer.from('too-large'), 'path', 'application/pdf'),
