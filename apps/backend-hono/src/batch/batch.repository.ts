...
export class BatchRepository {
  ...
  async getWorkspace(id: string) {
    const [row] = await db
      .select({
        ...
        paidEnrollments:
          sql<number>`count(${pesertaBatch.id}) filter (where ${pesertaBatch.paymentStatus} = 'paid')`.mapWith(
            Number,
          ),
        pendingPayments:
          sql<number>`count(${pesertaBatch.id}) filter (where ${pesertaBatch.paymentStatus} = 'pending')`.mapWith(
            Number,
          ),
      })
      .from(batchTraining)
  ...
  async findAll() {
    return db
      .select({
        ...
        trainerPhoto: trainer.fotoUrl,
        courseName: courses.title,
        participantsCount: sql<number>`count(${pesertaBatch.id})`.mapWith(Number),
      })
      .from(batchTraining)
      .leftJoin(trainer, eq(batchTraining.trainerId, trainer.id))
      .leftJoin(courses, eq(batchTraining.courseId, courses.id))
  ...
  async assignPeserta(batchId: string, pesertaId: string) {
    const [existing] = await db
      .select()
      .from(pesertaBatch)
      .where(and(eq(pesertaBatch.batchId, batchId), eq(pesertaBatch.pesertaId, pesertaId)))
      .limit(1)
    if (existing) return existing
    const [row] = await db
      .insert(pesertaBatch)
      .values({ batchId, pesertaId, status: 'registered' })
      .returning()
  ...
  async updateStatus(input: { batchId: string; pesertaId: string; status: string }) {
    const [existing] = await db
      ...
      .from(pesertaBatch)
      .where(
        and(eq(pesertaBatch.batchId, input.batchId), eq(pesertaBatch.pesertaId, input.pesertaId)),
      )
      .limit(1)
    ...
    const [row] = await db
      ...
      .where(eq(pesertaBatch.id, existing.id))
      .returning()
  ...
  async getCurriculum(batchId: string) {
    ...
    if (!batch?.courseId)
      return {
        batchId,
        courseTitle: batch?.namaBatch ?? 'Unknown Batch',
        modules: [],
        message: 'No course linked to this batch.',
      }
    ...
    for (const chapter of chapterList) {
      ...
      modules.push({
        id: chapter.id,
        title: chapter.title,
        duration: `${lessonList.reduce((acc, lesson) => acc + (Number.parseInt(lesson.duration ?? '0') || 0), 0)} mins`,
        content: `Contains ${lessonList.length} lessons.`,
