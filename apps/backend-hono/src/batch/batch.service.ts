...
type BatchRepositoryLike = {
  ...
  getActiveTiersForPublish(
    batchId: string,
  ): Promise<Array<{ price: number | string | null; scalevSyncStatus: string | null }>>
...
export class BatchService {
  ...
  constructor(
    deps: {
      ...
      fetcher?: (input: string, init?: RequestInit) => Promise<{ url: string }>
    } = {},
  ) {
  ...
  async create(input: Record<string, unknown>) {
    ...
    return this.repository.create({
      ...input,
      slug,
      tanggal: input.tanggal ? new Date(String(input.tanggal)) : new Date(),
    })
  }
  ...
  async listForUser(user: { id: string; role?: string }) {
    ...
    return rows.map((batch) => ({
      ...batch,
      journey: calculateJourney(batch.tanggal as Date, batch.tanggalSelesai as Date | null),
    }))
  }

  async assignPeserta(batchId: string, input: { pesertaIds: string[] }) {
    return Promise.all(
      input.pesertaIds.map((pesertaId) => this.repository.assignPeserta(batchId, pesertaId)),
    )
  }
  ...
  async publish(batchId: string) {
    ...
    if (activeTiers.some((tier) => Number(tier.price ?? 0) <= 0))
      throw new Error('BATCH_TIER_PRICE_REQUIRED')
    if (activeTiers.some((tier) => tier.scalevSyncStatus !== 'synced'))
