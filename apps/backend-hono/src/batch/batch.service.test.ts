...
describe('BatchService', () => {
  ...
  it('creates batch through repository', async () => {
    const service = new BatchService({
      repository: {
        create: async (input: unknown) => ({ id: 'batch_1', ...(input as object) }),
      } as any,
    })
  ...
  it('resolves map URL with fallback', async () => {
    const service = new BatchService({
      fetcher: async () => ({ url: 'https://maps.google.com/final' }) as Response,
    })

    expect(await service.resolveMapUrl('https://maps.app.goo.gl/x')).toEqual({
      finalUrl: 'https://maps.google.com/final',
    })
  })
    ...
    const service = new BatchService({
      repository: {
        ...
        getActiveTiersForPublish: async () => [
          { id: 'tier_1', price: 100000, scalevSyncStatus: 'synced' },
        ],
  ...
  it('blocks publish when an active tier has no positive price', async () => {
    const service = new BatchService({
      repository: {
        findById: async () => ({ id: 'batch_1', status: 'draft' }),
        getActiveTiersForPublish: async () => [
          { id: 'tier_1', price: 0, scalevSyncStatus: 'synced' },
        ],
      } as any,
    })
  ...
  it('blocks publish when active tiers are not synced', async () => {
    const service = new BatchService({
      repository: {
        findById: async () => ({ id: 'batch_1', status: 'draft' }),
        getActiveTiersForPublish: async () => [
          { id: 'tier_1', price: 100000, scalevSyncStatus: 'not_synced' },
        ],
      } as any,
    })
