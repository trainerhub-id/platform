...
const createBatchSchema = z
  .object({ namaBatch: z.string().min(1), tanggal: z.string().optional() })
  .passthrough()
const assignPesertaSchema = z.object({ pesertaIds: z.array(z.string().min(1)).min(1) })
const updateStatusSchema = z.object({
  pesertaId: z.string().min(1),
...
export function createBatchRoutes(service: BatchServiceLike = new BatchService()) {
  ...
  app.patch('/batch/:id', requireAuth, requireRole(['admin']), async (c) =>
    c.json({ batch: await service.update(c.req.param('id'), await c.req.json()) }),
  )
  app.delete('/batch/:id', requireAuth, requireRole(['admin']), async (c) =>
    c.json(await service.remove(c.req.param('id'))),
  )
  ...
  app.get('/batch/:id/peserta', requireAuth, requireRole(['admin']), async (c) =>
    c.json({ peserta: await service.getPesertaInBatch(c.req.param('id')) }),
  )
  app.get('/admin/batches/:id/workspace', requireAuth, requireRole(['admin']), async (c) =>
    c.json({ batch: await service.getWorkspace(c.req.param('id')) }),
  )
  app.post('/admin/batches/:id/publish', requireAuth, requireRole(['admin']), async (c) =>
    c.json({ batch: await service.publish(c.req.param('id')) }),
  )
  app.get('/batch/:id/curriculum', requireAuth, requireRole(['peserta', 'admin']), async (c) =>
    c.json(await service.getCurriculum(c.req.param('id'))),
  )
  app.post('/batch/resolve-map-url', requireAuth, requireRole(['admin']), async (c) =>
    c.json(await service.resolveMapUrl((await c.req.json()).url)),
  )
