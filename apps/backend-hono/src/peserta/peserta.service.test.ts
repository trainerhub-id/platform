...
describe("PesertaService", () => {
	it("creates peserta profile for user", async () => {
		...
		const profile = await service.create("user_1", { nama: "Budi", email: "budi@example.com", noWa: "081234567890", tShirtSize: "L" });
	...
	it("falls back to email and links profile", async () => {
		...
		const service = new PesertaService({
			repository: {
				...
				findByEmail: async () => ({ id: "peserta_1", clerkId: "old", nama: "Budi", email: "budi@example.com" }),
				linkClerkId: async () => {
					linked = true;
				},
			} as any,
