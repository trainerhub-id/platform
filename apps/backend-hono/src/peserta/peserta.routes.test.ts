...
describe("peserta routes", () => {
	it("creates profile for authenticated peserta", async () => {
		...
		app.route("/", createPesertaRoutes({ create: async (userId: string, input: Record<string, unknown>) => ({ id: "peserta_1", clerkId: userId, ...input }) } as any));

		const res = await app.request("/peserta/create", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ nama: "Budi", email: "budi@example.com" }) });
	...
	it("returns authenticated profile", async () => {
		...
		app.route("/", createPesertaRoutes({ getProfile: async () => ({ id: "peserta_1", nama: "Budi" }) } as any));
