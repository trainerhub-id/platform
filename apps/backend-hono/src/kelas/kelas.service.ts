...
import { batchTiers, chapters, courses, lessons, peserta, pesertaBatch, pesertaCourseProgress } from "../db/schema";
...
export class KelasService {
	...
	async getAllCourses(pesertaId?: string) {
		...
		if (!pesertaId) {
			return allCourses.map((course) => ({ ...course, hasAccess: true, progress: 0, completedLessons: 0, totalLessons: 0 }));
		}
		...
		return allCourses.map((course) => {
			...
			return {
				...
				progress: hasAccess && courseStats.total > 0 ? Math.round((courseStats.completed / courseStats.total) * 100) : 0,
			};
		});
	}
	...
	async getCourseDetail(courseId: string, pesertaId?: string) {
		...
		const courseChapters = await db.select().from(chapters).where(eq(chapters.courseId, courseId)).orderBy(chapters.orderIndex);
		const chapterIds = courseChapters.map((chapter) => chapter.id);
		const courseLessons = chapterIds.length > 0 ? await db.select().from(lessons).where(inArray(lessons.chapterId, chapterIds)).orderBy(lessons.orderIndex) : [];
		const progressRows = pesertaId && courseLessons.length > 0
			? await db.select().from(pesertaCourseProgress).where(and(eq(pesertaCourseProgress.pesertaId, pesertaId), inArray(pesertaCourseProgress.lessonId, courseLessons.map((lesson) => lesson.id))))
			: [];
		...
		for (const lesson of courseLessons) {
			...
			lessonsByChapter.set(lesson.chapterId, [...(lessonsByChapter.get(lesson.chapterId) ?? []), enriched]);
		}

		return { ...course, chapters: courseChapters.map((chapter) => ({ ...chapter, lessons: lessonsByChapter.get(chapter.id) ?? [] })) };
	}
	...
	async updateLessonProgress(pesertaId: string, lessonId: string, input: LessonProgressInput) {
		const [existing] = await db
			...
			.where(and(eq(pesertaCourseProgress.pesertaId, pesertaId), eq(pesertaCourseProgress.lessonId, lessonId)))
			.limit(1);
		...
		if (existing) {
			const [row] = await db.update(pesertaCourseProgress).set(values).where(eq(pesertaCourseProgress.id, existing.id)).returning();
			return row;
		}

		const [row] = await db
			.insert(pesertaCourseProgress)
			.values({ pesertaId, lessonId, status: input.status ?? "belum-mulai", videoProgress: input.videoProgress ?? 0, completedAt: input.status === "selesai" ? new Date() : null })
			.returning();
		return row;
	}
	...
	private async getAccessibleCourseIds(pesertaId: string) {
		...
		return new Set(enrollments.flatMap((enrollment) => Array.isArray(enrollment.courseIds) ? enrollment.courseIds : []));
	}

	private async getCourseStats(pesertaId: string, courseIds: string[]) {
		const stats = new Map<string, { total: number; completed: number }>();
		if (courseIds.length === 0) return stats;
		const courseChapters = await db.select({ id: chapters.id, courseId: chapters.courseId }).from(chapters).where(inArray(chapters.courseId, courseIds));
		if (courseChapters.length === 0) return stats;
		const chapterIds = courseChapters.map((chapter) => chapter.id);
		const courseLessons = await db.select({ id: lessons.id, chapterId: lessons.chapterId }).from(lessons).where(inArray(lessons.chapterId, chapterIds));
		...
		const progressRows = await db
			...
			.from(pesertaCourseProgress)
			.where(and(eq(pesertaCourseProgress.pesertaId, pesertaId), inArray(pesertaCourseProgress.lessonId, courseLessons.map((lesson) => lesson.id))));
		const progressMap = new Map(progressRows.map((row) => [row.lessonId, row.status]));
		const chapterCourseMap = new Map(courseChapters.map((chapter) => [chapter.id, chapter.courseId]));
		for (const lesson of courseLessons) {
