...
class FakeDocumentRepository {
  doc: any = {
    ...
    ownerUserId: 'user_1',
    flow: 'master',
    masterJson: { brainstorming_master: { organization_focus: 'Digital Marketing' } },
  }
  updated: any = null
  async findById() {
    return this.doc
...
class FakeFieldStateRepository {
  ...
  async upsert(input: any) {
    this.states = this.states.filter(
      (state) =>
        !(
          state.flow === input.flow &&
          state.phaseKey === input.phaseKey &&
  ...
  async getUnitDetail() {
    return {
      ...
      elements: [
        {
          element_number: 1,
          element_text: 'Menyiapkan kampanye',
          kuk: [{ kuk_code: '1.1', kuk_text: 'Tujuan kampanye ditentukan.' }],
        },
      ],
...
describe('skkni routes', () => {
  ...
  it('selects unit and updates Master readiness state', async () => {
    ...
    const unitDetail = fields.states.find(
      (state) => state.phaseKey === 'unit_selection' && state.fieldKey === 'unit_detail',
    )
    const competencyMap = fields.states.find(
      (state) => state.phaseKey === 'competency_map' && state.fieldKey === 'skkni_map',
    )
  ...
  it('selects unit and updates Trainer readiness state', async () => {
    ...
    docs.doc = {
      ...
      ownerUserId: 'user_1',
      flow: 'trainer',
      masterJson: { brainstorming: { expertise: 'Digital Marketing' } },
    }
    const fields = new FakeFieldStateRepository()
    const app = new Hono<{ Variables: { user: { id: string } } }>()
    app.use('*', async (c, next) => {
