...
export function createSkkniRoutes(deps: SkkniRouteDeps = {}) {
	...
	app.post("/api/documents/:documentId/skkni/search", requireAuth, async (c) => {
		...
		if (!isDocumentOwner(doc, userId)) return errorResponse(c, 403, "FORBIDDEN", "Document belongs to another user");
		if (doc.flow !== "master" && doc.flow !== "trainer") return errorResponse(c, 400, "FLOW_NOT_SUPPORTED", "Only master/trainer SKKNI search is supported");
	...
	app.post("/api/documents/:documentId/skkni/select", requireAuth, async (c) => {
		...
		if (!doc) return errorResponse(c, 404, "DOCUMENT_NOT_FOUND", "Document not found");
		if (!isDocumentOwner(doc, userId)) return errorResponse(c, 403, "FORBIDDEN", "Document belongs to another user");
		if (doc.flow !== "master" && doc.flow !== "trainer") return errorResponse(c, 400, "FLOW_NOT_SUPPORTED", "Only master/trainer SKKNI selection is supported");
		...
		const readiness = flow === "master" ? buildMasterReadiness(states) : buildTrainerReadiness(states);
		const currentPhase = flow === "master" ? getNextMasterPhase(states) : getNextTrainerPhase(states);
