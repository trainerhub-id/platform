...
export class PaymentRepository {
	async getBatchBySlugOrId(value: string) {
		const [bySlug] = await db.select().from(batchTraining).where(eq(batchTraining.slug, value)).limit(1);
	...
	async getTiersByBatch(batchId: string) {
		return db.select().from(batchTiers).where(and(eq(batchTiers.batchId, batchId), eq(batchTiers.isActive, true))).orderBy(batchTiers.orderIndex);
	}
	...
	async getTierBySlugOrId(batchId: string, tierSlugOrId: string) {
		const [tier] = await db
			.select()
			.from(batchTiers)
			.where(and(eq(batchTiers.batchId, batchId), or(eq(batchTiers.slug, tierSlugOrId), eq(batchTiers.id, tierSlugOrId)), eq(batchTiers.isActive, true)))
			.limit(1);
		return tier ?? null;
	}
	...
	async deleteTier(tierId: string) {
		await db.update(batchTiers).set({ isActive: false, updatedAt: new Date() }).where(eq(batchTiers.id, tierId));
	}
	...
	async findPaymentByEmailAndBatch(email: string, batchId: string) {
		const [row] = await db
			...
			.where(and(eq(paymentSessions.email, email), eq(paymentSessions.batchId, batchId), or(eq(paymentSessions.status, "pending"), eq(paymentSessions.status, "paid"))))
			.limit(1);
		return row ?? null;
	}

	async getPaymentSessionById(sessionId: string) {
		const [row] = await db.select().from(paymentSessions).where(eq(paymentSessions.id, sessionId)).limit(1);
		return row ?? null;
	}
	...
	async markClaimTokenUsed(sessionId: string) {
		await db.update(paymentSessions).set({ claimTokenUsed: true, updatedAt: new Date() }).where(eq(paymentSessions.id, sessionId));
	}

	async updatePaymentSessionUrl(sessionId: string, paymentUrl: string) {
		await db.update(paymentSessions).set({ paymentUrl, updatedAt: new Date() }).where(eq(paymentSessions.id, sessionId));
	}
	...
	async getPaymentsByBatch(batchId: string) {
		return db.select().from(paymentSessions).where(eq(paymentSessions.batchId, batchId)).orderBy(desc(paymentSessions.createdAt));
	}
}
