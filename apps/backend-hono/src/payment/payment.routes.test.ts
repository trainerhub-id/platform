...
describe('payment routes', () => {
  ...
  it('checks duplicates', async () => {
    ...
    const res = await app.request('/public/register/check-duplicate', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ email: 'budi@example.com', batchSlug: 'batch-q1' }),
    })
    const body = await res.json()
  ...
  it('claims paid sessions', async () => {
    const app = createPaymentRoutes({
      claimPayment: async (sessionId: string, token?: string) => ({
  ...
  it('refreshes payment sessions on check', async () => {
    const app = createPaymentRoutes({
      refreshScalevPaymentStatus: async (
        sessionId: string,
        token?: string,
        forceCheck?: boolean,
  ...
  it('creates admin batch tiers behind the admin role guard', async () => {
    ...
    app.route(
      '/',
      createPaymentRoutes({
        createTier: async (batchId: string, input: any) => ({
          id: 'tier_1',
          batchId,
          price: input.price,
        }),
