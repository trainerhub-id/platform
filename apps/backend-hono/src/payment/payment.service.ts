...
type PaymentRepositoryLike = {
	...
	getPublicBatchInfo(batchSlug: string): Promise<{ batch: Record<string, unknown>; tiers: Array<Record<string, unknown>> }>;
	...
	updateSessionFromScalev(sessionId: string, input: { provider: "scalev"; providerOrderId: number; providerOrderCode: string; providerReferenceId: string | null; providerPaymentMethod: string; providerSubPaymentMethod: string | null; providerCheckoutUrl: string | null; providerQrString: string | null; providerVaNumber: string | null; providerExpiresAt: Date | null; providerPayload: Record<string, unknown>; status: string }): Promise<PaymentSessionRecord | null>;
	getTiersByBatch(batchId: string): Promise<TierRecord[]>;
	getTierById(tierId: string): Promise<TierRecord | null>;
	getTierTemplateById(id: string): Promise<{ id: string; name: string; slug: string; description?: string | null; defaultCourseIds?: string[] | null; defaultAiFeatures?: string[] | null; defaultBenefits?: string[] | null } | null>;
...
type ScalevLike = Pick<ScalevService, "listStoresSimplified" | "findProductByVariantUniqueId" | "upsertProduct" | "deleteProduct" | "createOrder" | "createPaymentForOrder" | "getOrder" | "checkOrderPayment" | "normalizePaymentSession">;
...
export class PaymentService {
	...
	async checkDuplicate(input: { email: string; batchSlug: string }) {
		...
		if (existing.status === "paid") return { isDuplicate: true, message: "Email ini sudah terdaftar di batch ini" };
	...
	async getResolvedTier(tierId: string) {
		const tier = await this.repository.getTierById(tierId);
		if (!tier) throw new Error("TIER_NOT_FOUND");
		if (!tier.tierTemplateId) return { ...tier, resolvedCourseIds: tier.courseIds ?? null, resolvedAiFeatures: tier.aiFeatures ?? null, resolvedBenefits: tier.benefits ?? null, tierTemplate: null };
	...
	async createRegistration(input: CreateRegistrationInput) {
		...
		if (this.paymentProvider === "scalev") {
			...
			try {
				const orderId = Number((order as { data?: { id?: unknown }; id?: unknown })?.data?.id ?? (order as { id?: unknown })?.id);
			...
			const updated = await this.repository.updateSessionFromScalev(sessionId, this.toScalevSessionUpdate(normalized));
			return this.buildCheckoutResponse(updated ?? { id: sessionId, email: input.email, amount: tier.price, status: normalized.status, ...this.sessionFieldsFromScalev(normalized) });
		}
	...
	async refreshScalevPaymentStatus(sessionId: string, claimToken?: string, forceCheck = true) {
		...
		if (!session) throw new Error("PAYMENT_SESSION_NOT_FOUND");
		this.assertClaimToken(session, claimToken);
		if (session.provider !== "scalev" || !session.providerOrderId) return this.buildCheckoutResponse(session);

		const order = await this.scalev.getOrder(session.providerOrderId);
		let normalized = this.scalev.normalizePaymentSession(order);
		let updated = await this.repository.updateSessionFromScalev(sessionId, this.toScalevSessionUpdate(normalized));
	...
	private resolveScalevTierConfig(tier: TierRecord) {
		...
		const bundlePriceOptionUniqueId = tier.scalevBundlePriceOptionUniqueId || env.SCALEV_BUNDLE_PRICE_OPTION_UNIQUE_ID || null;
	...
	private async resolveTierInput(input: TierInput): Promise<TierInput> {
		if (!input.tierTemplateId) {
			return { ...input, name: input.name ?? "Tier", slug: input.slug ?? this.createSlug(input.name ?? "tier") };
		}
	...
	private buildCheckoutResponse(session: PaymentSessionRecord) {
		...
		return {
			...
			expiresAt: session.providerExpiresAt ? new Date(session.providerExpiresAt).toISOString() : null,
