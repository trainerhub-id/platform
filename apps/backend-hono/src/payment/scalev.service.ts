...
export class ScalevService {
	...
	async listStoresSimplified(): Promise<ScalevStoreSummary[]> {
		const response = await this.request<{ data?: { results?: ScalevStoreSummary[] } }>("/v2/stores/simplified", { method: "GET" });
		return response?.data?.results || [];
	}

	async listProductsSimplified(): Promise<ScalevProductSummary[]> {
		const response = await this.request<{ data?: { results?: ScalevProductSummary[] } }>("/v2/products/simplified", { method: "GET" });
		return response?.data?.results || [];
	}

	async findProductByVariantUniqueId(variantUniqueId: string): Promise<ScalevProductSummary | null> {
		if (!variantUniqueId) return null;
		const products = await this.listProductsSimplified();
		return products.find((product) => Array.isArray(product.variants) && product.variants.some((variant) => variant.unique_id === variantUniqueId)) || null;
	}
	...
	async upsertProduct(params: UpsertScalevProductParams): Promise<unknown> {
		const body = {
			...
			variants: [{ id: params.existingVariantId || undefined, name: params.name, price: params.price.toFixed(2), pricing_type: "one_time" }],
		};
		if (params.existingProductId) return this.request(`/v2/products/${params.existingProductId}`, { method: "PATCH", body: JSON.stringify(body) });
	...
	async createOrder(params: CreateScalevOrderParams): Promise<unknown> {
		return this.request("/v2/order", {
			...
			body: JSON.stringify({
				...
				ordervariants: params.orderVariants.map((variant) => ({ variant_unique_id: variant.variantUniqueId, quantity: variant.quantity })),
				notes: params.notes,
	...
	async checkOrderPayment(orderId: number): Promise<unknown> {
		try {
			...
			if (error instanceof Error && error.message === "Order is not yet paid.") return { status: "Pending", code: 400, error: error.message };
			throw error;
		}
	}

	mapScalevStatus(order: unknown): ScalevPaymentStatus {
		const source = order as { data?: Record<string, unknown>; payment_status?: unknown; pg_payment_info?: { status?: unknown } };
		const data = source.data ?? source;
		const paymentStatus = String(data.payment_status || "").toLowerCase();
		const providerStatus = String((data.pg_payment_info as { status?: unknown } | undefined)?.status || "").toLowerCase();
	...
	normalizePaymentSession(orderResponse: unknown, paymentResponse?: unknown): ScalevPaymentSession {
		...
		const payment = paymentEnvelope?.data || (order.pg_payment_info as Record<string, unknown> | undefined) || {};
		const paymentMethod = (payment.payment_method as Record<string, unknown> | undefined) || {};
		const virtualAccount = (paymentMethod.virtual_account as Record<string, unknown> | undefined) || {};
		...
		const actions = Array.isArray(payment.actions) ? payment.actions as Array<{ url?: string }> : [];

		return {
			provider: "scalev",
			providerOrderId: Number(order.id),
			providerOrderCode: String(order.order_id ?? ""),
			providerReferenceId: (order.pg_reference_id as string | undefined) || (payment.reference_id as string | undefined) || null,
			...
			checkoutUrl: (payment.invoice_url as string | undefined) || actions[0]?.url || (order.payment_url as string | undefined) || null,
			qrString: (qrProps.qr_string as string | undefined) || null,
			vaNumber: (vaProps.virtual_account_number as string | undefined) || null,
			expiresAt: (vaProps.expires_at as string | undefined) || (qrProps.expires_at as string | undefined) || (payment.expiry_date as string | undefined) || null,
