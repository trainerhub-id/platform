...
describe('PaymentService', () => {
  it('returns public batch info', async () => {
    const service = new PaymentService({
      repository: {
        getPublicBatchInfo: async () => ({ batch: { id: 'batch_1' }, tiers: [] }),
      } as any,
    })
  ...
  it('creates a manual registration session using the tier price from storage', async () => {
    ...
    expect(result).toEqual({
      ...
      status: 'pending',
      paymentUrl:
        'https://app.example/payment/success?session=session_1&ref=TH-batch_1-1000&email=budi%40example.com&amount=250000&batchId=batch_1&tierId=tier_1&token=token_1',
    ...
    const service = new PaymentService({
      ...
      scalev: {
        createOrder: async () => {
          ...
          return {
            data: { id: 42, order_id: 'ORD-42', payment_method: 'qris', payment_status: 'pending' },
          }
        },
        createPaymentForOrder: async () => {
          ...
          return {
            data: {
              invoice_url: 'https://pay.example/checkout',
              payment_method: { qr_code: { channel_properties: { qr_string: 'QRDATA' } } },
            },
          }
        },
  ...
  it('refreshScalevPaymentStatus marks linked enrollment paid when provider reports paid', async () => {
    ...
    const service = new PaymentService({
      ...
      scalev: {
        getOrder: async () => ({
          data: { id: 42, order_id: 'ORD-42', payment_method: 'qris', payment_status: 'paid' },
        }),
        checkOrderPayment: async () => ({ ok: true }),
        normalizePaymentSession: () => ({
          provider: 'scalev',
