...
describe('GenerationWorkerService', () => {
  it('registers all generation job handlers', async () => {
    ...
    const worker = new GenerationWorkerService({
      boss: boss as any,
      documentGenerator: {
        generateFromJob: async (payload: unknown) => calls.push(payload),
      } as any,
    })
  ...
  it('stops boss', async () => {
    ...
    const worker = new GenerationWorkerService({
      boss: boss as any,
      documentGenerator: { generateFromJob: async () => [] } as any,
    })
  ...
  it('stores generated render results from job handler', async () => {
    ...
    const worker = new GenerationWorkerService({
        ...
        generateFromJob: async () => [
          {
            ...
            bytes: new TextEncoder().encode('docx'),
      ...
      objectStorage: {
        uploadBuffer: async (_buffer: Uint8Array, path: string) => ({
          key: `${path}/bukti-1.docx`,
          url: 'signed',
        }),
      } as any,
    ...
    expect((savedFiles[0] as { documentType: string; filePath: string }).documentType).toBe(
      'bukti-1',
    )
