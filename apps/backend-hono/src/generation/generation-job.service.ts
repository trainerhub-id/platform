...
export type GenerationJobType = "generate-master-documents" | "generate-trainer-documents" | "regenerate-document" | "render-single-document";
...
type JobsRepository = {
	...
	create(input: { documentId: string; jobType: string; requestKey: string; input: unknown }): Promise<GenerationJobRecord>;
...
class DbGenerationJobsRepository implements JobsRepository {
	async listByDocument(documentId: string) {
		return db.select().from(generationJobs).where(eq(generationJobs.documentId, documentId)).orderBy(desc(generationJobs.createdAt));
	}
	...
	async findByTypeAndRequestKey(jobType: string, requestKey: string) {
		const [job] = await db.select().from(generationJobs).where(and(eq(generationJobs.jobType, jobType), eq(generationJobs.requestKey, requestKey))).limit(1);
		return job ?? null;
	}
...
type EnqueueFn = (jobName: string, payload: unknown, options: { singletonKey: string }) => Promise<string | null>;
...
export class GenerationJobService {
	...
	constructor(deps: { jobs?: JobsRepository; enqueue?: EnqueueFn; boss?: PgBoss } = {}) {
		...
		this.enqueue = deps.enqueue ?? (async (jobName, payload, options) => {
			if (!deps.boss) return null;
			const id = await deps.boss.send(jobName, payload as object, { singletonKey: options.singletonKey } as never);
	...
	async enqueueGeneration(input: GenerationRequest) {
		...
		const job = await this.jobs.create({ documentId: input.documentId, jobType: input.jobType, requestKey, input });
		const bossJobId = await this.enqueue(input.jobType, input, { singletonKey: requestKey });
