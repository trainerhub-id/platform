...
describe("buildGenerationRequestKey", () => {
	it("uses provided request key when present", () => {
		expect(buildGenerationRequestKey({ documentId: "doc_1", jobType: "generate-master-documents", documentTypes: ["bukti-1"], requestKey: "client-key" })).toBe("client-key");
	});

	it("creates deterministic key with sorted document types", () => {
		expect(buildGenerationRequestKey({ documentId: "doc_1", jobType: "generate-master-documents", documentTypes: ["bukti-2", "bukti-1"] })).toBe("generate-master-documents:doc_1:bukti-1,bukti-2");
	});
});
...
describe("GenerationJobService", () => {
	it("returns existing job for same job type and request key without enqueue", async () => {
		...
		const job = await service.enqueueGeneration({ documentId: "doc_1", jobType: "generate-master-documents", documentTypes: ["bukti-1"], requestKey: "key_1" });
	...
	it("requeues an existing job by id", async () => {
		...
		const existing = {
			...
			status: "failed",
			requestKey: "key_1",
			input: { documentId: "doc_1", jobType: "generate-master-documents", documentTypes: ["bukti-1"], requestKey: "key_1" },
		};
