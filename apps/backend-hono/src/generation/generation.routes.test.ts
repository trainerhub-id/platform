...
class FakeDocumentRepository {
  doc = {
    ...
    ownerUserId: 'user_1',
    flow: 'master',
    readiness: { ready: false, missing: ['unit_selection.selected_unit_code'] },
    masterJson: {},
  }
  async findById() {
    return this.doc
...
describe('generation routes', () => {
  it('blocks unauthenticated generate request', async () => {
    ...
    const res = await app.request('/api/documents/doc_1/generate', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ documentTypes: ['bukti-1'] }),
    })
  ...
  it('redirects generated file downloads to an object storage signed URL', async () => {
      ...
      createGenerationRoutes({
        ...
        generatedFiles: {
          findById: async () => ({
            id: 'file_1',
            documentId: 'doc_1',
            filePath: 'generated/doc_1/job_1/123.docx',
            filename: 'bukti-1.docx',
    ...
    expect(res.headers.get('location')).toBe(
      'https://storage.example/generated/doc_1/job_1/123.docx?signed=1',
    )
  })
  ...
  it('enqueues a single document generation route', async () => {
    ...
    app.route(
      ...
      createGenerationRoutes({
        ...
        generationJobs: {
          enqueueGeneration: async (input: any) => ({ id: 'job_1', input }),
        } as any,
      }),
    )
  ...
  it('returns a generation job by id', async () => {
    ...
    app.route(
      ...
      createGenerationRoutes({
        documents: new ReadyFakeDocumentRepository() as any,
        generationJobs: {
          findById: async () => ({ id: 'job_1', documentId: 'doc_1', status: 'queued' }),
        } as any,
      }),
    )
