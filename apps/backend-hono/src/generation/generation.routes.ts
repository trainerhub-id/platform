...
type GenerationRoutesDeps = {
  ...
  generationJobs?: Pick<
    ...
    'enqueueGeneration' | 'listByDocument' | 'findById' | 'retry'
  >
...
export function createGenerationRoutes(deps: GenerationRoutesDeps = {}) {
  ...
  async function requireOwnedDocument(c: Context<{ Variables: GenerationVariables }>) {
    ...
    if (!documentId)
      return { error: errorResponse(c, 400, 'VALIDATION_ERROR', 'Document id is required') }
  ...
  async function enqueueForDocument(
    ...
    if (!readinessReady(doc.readiness)) {
      const missing =
        typeof doc.readiness === 'object' && doc.readiness
          ? (doc.readiness as { missing?: unknown }).missing
          : undefined
      return c.json(
        {
          error: {
    ...
    const jobType: GenerationJobType =
      doc.flow === 'trainer' ? 'generate-trainer-documents' : 'generate-master-documents'
