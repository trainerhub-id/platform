...
type DokumenServiceLike = Pick<DokumenService, "getKategori" | "getJenisByKategori" | "getStatus" | "upload" | "delete">;
...
export function createDokumenRoutes(service: DokumenServiceLike = new DokumenService()) {
	...
	app.get("/dokumen/kategori", requireAuth, requireRole(["peserta", "admin"]), async (c) => c.json(await service.getKategori()));
	app.get("/dokumen/jenis/:kategoriId", requireAuth, requireRole(["peserta", "admin"]), async (c) => c.json(await service.getJenisByKategori(c.req.param("kategoriId"))));
	...
	app.post("/dokumen/upload", requireAuth, requireRole(["peserta"]), async (c) => {
		...
		if (typeof jenisId !== "string" || !z.string().uuid().safeParse(jenisId).success) return errorResponse(c, 400, "VALIDATION_ERROR", "valid jenisId is required");
		const user = c.get("user") as UserLike;
