...
export class DokumenService {
  ...
  async getStatus(userId: string) {
    ...
    return Promise.all(
      rows.map(async (row) => ({ ...row, fileUrl: await this.storage.getPublicUrl(row.fileUrl) })),
    )
  }
  ...
  async upload(file: File, jenisId: string, userId: string) {
    ...
    const [jenis] = await db
      .select()
      .from(dokumenJenis)
      .where(eq(dokumenJenis.id, jenisId))
      .limit(1)
    ...
    const [existing] = await db
      .select()
      .from(dokumenPeserta)
      .where(and(eq(dokumenPeserta.pesertaId, profile.id), eq(dokumenPeserta.jenisId, jenisId)))
      .limit(1)
    if (existing) {
      const [row] = await db
    ...
    const [row] = await db
      ...
      .values({ pesertaId: profile.id, jenisId, fileUrl: uploaded.key, status: 'pending' })
      .returning()
    return row
  }
  ...
  async delete(id: string, userId: string) {
    ...
    await db
      ...
      .where(and(eq(dokumenPeserta.id, id), eq(dokumenPeserta.pesertaId, profile.id)))
    return { success: true }
  }
}
