...
describe('BNSP certificate migration', () => {
  it('uploads a BNSP certificate to object storage and creates row', async () => {
    ...
    const service = new CertificateService({
      ...
      storage: {
        upload: async (file: { originalname: string }, path: string) => ({
          key: `${path}/123-${file.originalname}`,
          url: 'signed',
        }),
        getPublicUrl: async (key: string) => `https://files.example/${key}`,
        buildPesertaSertifikatPath: (pesertaId: string) =>
          `storage/peserta/${pesertaId}/sertifikat`,
