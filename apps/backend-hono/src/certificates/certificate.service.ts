...
import { CertificatePdfService, type CertificatePdfData } from "./certificate-pdf.service";
import { buildCertificateNumber, CertificateRepository, mapCertificateListItem, mapValidationResult } from "./certificate.repository";
...
type CertificateRepositoryLike = {
	...
	getCourseProgress(pesertaId: string, courseId: string): Promise<{ progress: number; totalLessons: number; completedLessons: number }>;
	...
	createBnspCertificate(input: { pesertaId: string; nomorSertifikat?: string; lsp?: string; fileUrl: string }): Promise<CertificateRecord>;
	updateBnspCertificate(id: string, input: { nomorSertifikat?: string | null; lsp?: string | null; fileUrl: string }): Promise<CertificateRecord>;
};

type StorageLike = {
	uploadBuffer(buffer: Buffer, path: string, contentType: string): Promise<{ key: string; url?: string }>;
	upload(file: { originalname: string; buffer: Buffer | Uint8Array; mimetype: string; size: number }, path: string): Promise<{ key: string; url?: string }>;
...
export class CertificateService {
	...
	constructor(deps: { repository?: CertificateRepositoryLike; storage?: StorageLike; pdf?: Pick<CertificatePdfService, "render">; qr?: QrLike } = {}) {
	...
	async uploadBnspCertificate(
		...
		const upload = await this.storage.upload(file, this.storage.buildPesertaSertifikatPath(input.pesertaId));
		if (existing) {
	...
	async getMyCertificates(userId: string) {
		...
		return Promise.all(certificates.map((cert) => mapCertificateListItem({ ...cert, id: cert.id }, (key) => this.storage.getPublicUrl(key))));
	}
	...
	async getAllMyCertificates(userId: string) {
		...
		return Promise.all(certificates.map(async (cert) => ({ ...cert, fileUrl: cert.fileUrl ? await this.storage.getPublicUrl(cert.fileUrl) : null })));
	}
	...
	async getEligibleCourses(userId: string) {
		...
		return Promise.all(courses.map(async (course) => {
			...
			return { courseId: course.id, courseName: course.title, progress: Math.round(progress.progress), hasCertificate: !!certificate, certificateId: certificate?.id ?? null };
		}));
	}
	...
	async generateForCourse(pesertaId: string, pesertaName: string, courseId: string) {
		...
		if (existing) return mapCertificateListItem({ ...existing, id: existing.id }, (key) => this.storage.getPublicUrl(key));
		...
		const certificateNumber = buildCertificateNumber({ year, courseTitle: course.title, sequence: count + 1 });
		...
		const upload = await this.storage.uploadBuffer(pdf, this.storage.buildCertificatePath(pesertaId, courseId), "application/pdf");
		...
		return mapCertificateListItem({ ...certificate, id: certificate.id }, (key) => this.storage.getPublicUrl(key));
	}
}
