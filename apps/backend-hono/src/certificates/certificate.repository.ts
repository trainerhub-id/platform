...
import { chapters, courses, lessons, peserta, pesertaCourseProgress, sertifikat } from "../db/schema";
...
export function buildCertificateNumber(input: { year: number; courseTitle: string; sequence: number }): string {
	const courseCode = input.courseTitle.replace(/[^a-zA-Z]/g, "").substring(0, 3).toUpperCase() || "CRS";
...
export async function mapCertificateListItem(cert: CertificateLike & { id: string }, getPublicUrl: PublicUrlFn) {
	return {
		id: cert.id,
		certificateNumber: cert.certificateNumber,
		courseName: cert.courseName,
...
export class CertificateRepository {
	...
	async findByNumber(certificateNumber: string) {
		const [row] = await db.select().from(sertifikat).where(eq(sertifikat.certificateNumber, certificateNumber)).limit(1);
		return row ?? null;
	}
	...
	async findByCourseAndPeserta(pesertaId: string, courseId: string) {
		const [row] = await db
			.select()
			.from(sertifikat)
			.where(and(eq(sertifikat.pesertaId, pesertaId), eq(sertifikat.courseId, courseId), eq(sertifikat.type, "trainerhub")))
			.limit(1);
		return row ?? null;
	}
	...
	async listAllByPeserta(pesertaId: string) {
		return db.select().from(sertifikat).where(eq(sertifikat.pesertaId, pesertaId)).orderBy(sql`${sertifikat.createdAt} DESC`);
	}
	...
	async countCourseCertificatesForYear(courseId: string, year: number) {
		const [row] = await db
			...
			.from(sertifikat)
			.where(and(eq(sertifikat.courseId, courseId), sql`EXTRACT(YEAR FROM ${sertifikat.createdAt}) = ${year}`));
		return row?.count ?? 0;
	}
	...
	async createBnspCertificate(input: { pesertaId: string; nomorSertifikat?: string; lsp?: string; fileUrl: string }) {
		const [row] = await db
			.insert(sertifikat)
			.values({
				pesertaId: input.pesertaId,
	...
	async updateBnspCertificate(id: string, input: { nomorSertifikat?: string | null; lsp?: string | null; fileUrl: string }) {
		const [row] = await db
			.update(sertifikat)
			.set({ nomorSertifikat: input.nomorSertifikat ?? undefined, lsp: input.lsp ?? undefined, fileUrl: input.fileUrl, status: "issued", issuedDate: new Date(), updatedAt: new Date() })
	...
	async getCourseProgress(pesertaId: string, courseId: string) {
		...
		const completed = await db
			...
			.where(and(eq(pesertaCourseProgress.pesertaId, pesertaId), eq(pesertaCourseProgress.status, "selesai"), inArray(pesertaCourseProgress.lessonId, lessonIds)));
		return { totalLessons: lessonIds.length, completedLessons: completed.length, progress: (completed.length / lessonIds.length) * 100 };
	}
