...
function getPalette(templateName?: string | null) {
  ...
  if (normalized.includes('elegant'))
    return {
      frame: '#581c87',
      accent: '#c026d3',
      soft: '#fae8ff',
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            normalizeText(input.pesertaName, 'Peserta'),
          ),
          React.createElement(View, { style: styles.line }),
          React.createElement(
            Text,
            { style: styles.body },
            `Telah menyelesaikan pelatihan ${normalizeText(input.courseName, '- ')} dengan baik.`,
          ),
          React.createElement(
            View,
            { style: styles.card },
            ...
            React.createElement(
              Text,
              { style: styles.value },
              normalizeText(input.certificateNumber, '-'),
            ),
          ...
          input.qrCodeDataUri
            ? React.createElement(Image, { src: input.qrCodeDataUri, style: styles.qr })
            : null,
        ),
        React.createElement(
          Text,
          { style: styles.footer },
          `Diterbitkan oleh ${normalizeText(input.trainerName, 'TrainerHub Team')}`,
        ),
      ),
    ),
  )
