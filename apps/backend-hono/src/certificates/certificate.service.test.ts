...
describe("CertificateService", () => {
	...
	it("generates ReactPDF certificate for completed course", async () => {
		...
		const service = new CertificateService({
			...
			storage: {
				uploadBuffer: async (_buffer: Buffer, path: string, mimeType: string) => ({ key: `${path}/123.pdf`, url: `signed:${mimeType}` }),
		...
		expect(result.pdfUrl).toBe("https://files.example/storage/peserta/peserta_1/certificates/course_1/123.pdf");
	});
});
