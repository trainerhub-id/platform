...
export const useKelas = (courseId?: string) => {
    ...
    const markLessonComplete = async (lessonId: string, status: 'belum-mulai' | 'selesai') => {
        ...
        try {
            ...
            setSelectedKelas({
                ...
                completedChapters: updatedChapters.filter(ch => ch.lessons.every(l => l.status === 'selesai')).length
            });
    ...
    const saveVideoProgress = async (lessonId: string, progressInSeconds: number) => {
        ...
        try {
            await api.patch(
                `/kelas/${selectedKelas.id}/lesson/${lessonId}/progress`,
                { 
    ...
    const currentLessonDisplay = () => {
        ...
        const chapter = selectedKelas.chapters.find(ch => ch.lessons.some(l => l.id === activeLesson.id));
