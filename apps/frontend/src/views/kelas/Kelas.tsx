...
import { extractYouTubeId, extractGoogleDriveId, getGoogleDriveEmbedUrl, isYouTubeUrl, isGoogleDriveUrl } from 'src/utils/videoHelpers';
import './kelas.css';
...
const Kelas = () => {
  ...
  const fetchPlaybackToken = useCallback(async (lessonId: string) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || '/api'}/kelas/lessons/${lessonId}/playback-token`, {
        headers: {
  ...
  const handleCheckboxChange = () => {
    ...
    const newStatus = activeLesson.status === 'selesai' 
        ? 'belum-mulai' 
        : 'selesai';
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 shrink-0">
        <div className="flex items-center gap-3 flex-1">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-bold text-dark leading-tight">
              {selectedKelas.title}
            </h1>
        ...
        <div className="flex items-center gap-2">
          <div className="px-3 py-1.5 border border-ld rounded-lg flex items-center gap-2 text-xs text-bodytext bg-white">
            ...
            <span>Chapter : {selectedKelas.completedChapters}/{selectedKelas.totalChapters}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-4 flex-1 min-h-0">
        ...
        <div className={`${isSidebarOpen ? 'lg:col-span-8' : 'lg:col-span-12'} col-span-12 transition-all duration-300 h-auto lg:h-full flex flex-col lg:overflow-y-auto overflow-visible custom-scrollbar`}>
          ...
          <div className="relative w-full bg-black shadow-sm shrink-0 rounded-xl overflow-hidden aspect-video">
            {tokenLoading ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center bg-gray-900 text-white">
                <Icon icon="solar:videocamera-slash-linear" height={48} className="text-gray-500 mb-2" />
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-1 border-t border-gray-100 mt-2">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
                <label className="flex items-center gap-2 cursor-pointer select-none group">
                  ...
                  <span className={`text-sm ${activeLesson.status === 'selesai' ? 'text-dark font-medium' : 'text-bodytext'}`}>
                    {activeLesson.status === 'selesai' 
                ...
                <span className="text-sm font-medium text-dark">Chapter {currentChapterNum} dari {totalChapters}</span>
              </div>
            ...
            <CardBox className="h-full border-none shadow-sm flex flex-col overflow-hidden">
              <div className="flex items-center justify-between mb-4 px-1">
                ...
                <Button
                  ...
                  title="Tutup Sidebar"
                >
                  <Icon icon="solar:alt-arrow-down-linear" height={18} className="text-gray-500" />
                </Button>
              </div>
