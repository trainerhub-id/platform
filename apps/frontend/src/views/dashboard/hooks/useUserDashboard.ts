  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [profileResult, batchesResult, tasksResult, submissionsResult] =
        ...
        const submissionsRes =
          submissionsResult.status === 'fulfilled' ? submissionsResult.value.data : []
        ...
        const batches = Array.isArray(batchesRes)
          ? batchesRes
          : Array.isArray(batchesRes?.batches)
            ? batchesRes.batches
            : []
        ...
        setData({
          profile: profileRes,
          activeBatch,
          tasks: Array.isArray(tasksRes)
            ? tasksRes
            : Array.isArray(tasksRes?.tasks)
              ? tasksRes.tasks
              : [],
          submissions: Array.isArray(submissionsRes)
            ? submissionsRes
            : Array.isArray(submissionsRes?.submissions)
