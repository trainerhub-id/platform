...
const TestTableResponsive = () => {
    ...
    const columns = [
                ...
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200">
                        <img
                            ...
                            onError={(e) => {
                                (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${info.getValue()}&background=random`;
                            }}
                        />
                    </div>
        ...
        columnHelper.accessor('dataBnspStatus', {
            ...
            cell: (info: any) => {
                ...
                return (
                    <span className={`px-3 py-1 rounded-md text-xs font-medium ${bgClass}`}>
                        {status}
                    </span>
                );
            },
        ...
        columnHelper.display({
            ...
            cell: (info: any) => (
                <button
                    className="bg-[#B58E36] text-white text-xs px-4 py-2 rounded-lg flex items-center gap-1 hover:bg-[#94742C] transition-colors"
                >
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="text-bodytext border-0 card no-inset no-ring bg-white dark:bg-darkgray p-0 dark:shadow-dark-md shadow-md" style={{ borderRadius: '7px' }}>
                {/* Table Header */}
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 sm:p-6 border-t bg-gray-50/50">
                    <div className="text-xs sm:text-sm text-gray-500 text-center sm:text-left">
                        Menampilkan {table.getRowModel().rows.length} dari {table.getFilteredRowModel().rows.length} peserta
                    </div>
                        ...
                        <div className="flex items-center gap-1">
                            {Array.from({ length: table.getPageCount() }, (_, i) => i + 1).map((page) => (
                                <button
                                    ...
                                    className={`w-10 h-10 rounded-full font-medium flex items-center justify-center transition-all ${table.getState().pagination.pageIndex === page - 1
