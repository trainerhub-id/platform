...
const DaftarPeserta = () => {
    ...
    const activeFiltersCount = [paymentFilter, bnspFilter, portfolioFilter, certificateFilter, batchFilter].filter(f => f !== 'all').length;
    ...
    const columns = [
        ...
        columnHelper.accessor('paymentStatus', {
            ...
            cell: (info: any) => {
                ...
                return (
                    <span className={`px-2 py-1 rounded-md text-xs font-medium ${bgClass} capitalize inline-block`}>
                        {label}
                    </span>
                );
            },
                            ...
                            {table.getHeaderGroups().map(headerGroup => (
                                <tr key={headerGroup.id}>
                                    {headerGroup.headers.map(header => (
                                        <th key={header.id} className="text-left py-3 px-3 text-gray-500 font-medium text-xs">
