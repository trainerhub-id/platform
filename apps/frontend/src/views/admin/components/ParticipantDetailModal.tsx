...
export const ParticipantDetailModal = ({ open, onOpenChange, participantId }: ParticipantDetailModalProps) => {
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-gray-50 rounded-lg p-6 space-y-4">
                            <div className="flex items-center gap-4">
                                ...
                                <span className={`px-3 py-1 rounded-md text-xs font-medium ${data.profile?.dataBnspStatus === 'Lengkap' ? 'bg-green-50 text-green-600' : 'bg-yellow-50 text-yellow-600'}`}>
                                ...
                                <div className="bg-white border rounded-lg p-4">
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-medium">{data.profile.jk === 'L' ? 'Laki-laki' : data.profile.jk === 'P' ? 'Perempuan' : '-'}</p>
                                        </div>
                                        <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="bg-white border rounded-lg p-4 text-center">
                                <Icon icon="solar:document-text-bold" className="mx-auto text-primary mb-2" height={24} />
                                <p className="text-2xl font-bold text-dark">{data.documents.total}</p>
                                <p className="text-sm text-gray-500">Total Dokumen</p>
                            </div>
                            <div className="bg-white border rounded-lg p-4 text-center">
                                <Icon icon="solar:check-circle-bold" className="mx-auto text-green-500 mb-2" height={24} />
                                <p className="text-2xl font-bold text-dark">{data.documents.approved}</p>
                                <p className="text-sm text-gray-500">Approved</p>
                            </div>
                            <div className="bg-white border rounded-lg p-4 text-center">
                                <Icon icon="solar:clock-circle-bold" className="mx-auto text-yellow-500 mb-2" height={24} />
                        ...
                        <div>
                            ...
                            {documents.length > 0 ? (
                                    ...
                                    {documents.map((doc) => (
                                        <div key={doc.id} className="bg-white border rounded-lg p-4 flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                <Icon icon="solar:document-text-bold" className="text-gray-400" height={24} />
                                                <div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className={`px-2 py-1 rounded text-xs font-medium ${getBadgeClass(doc.status)}`}>
                                                    {doc.status === 'approved' ? 'Disetujui' : doc.status === 'revisi' ? 'Revisi' : 'Pending'}
                                                </span>
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    ...
                                                    <>
                                                        ...
                                                        >
                                                            {reviewingDoc === doc.id ? (
                                                                ...
                                                                <Icon icon="solar:check-circle-linear" height={16} className="mr-1" />
                                                            )}
                                                            Approve
                                                        </Button>
                                                        <Button
                                ...
                                <div className="bg-gray-50 rounded-lg p-6 text-center text-gray-500">
                                    <Icon icon="solar:folder-open-linear" height={32} className="mx-auto mb-2 text-gray-400" />
                        ...
                        <div>
                            ...
                            <div className="bg-white border rounded-lg p-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className={`px-3 py-1 rounded-md text-xs font-medium ${data.hasCertificate ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-600'}`}>
                        ...
                        <div>
                            ...
                            <div className="bg-white border rounded-lg p-4">
                                    ...
                                    <div>
                                        ...
                                        {bnspCertificate?.lsp && (
                                            <p className="text-xs text-gray-400">
                                                LSP: {bnspCertificate.lsp}
                                            </p>
                                        )}
                                    </div>
                                    <span className={`px-3 py-1 rounded-md text-xs font-medium ${bnspCertificate ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-600'}`}>
                                ...
                                <div className="mt-3 flex gap-2">
                                    ...
                                    <label className={`inline-flex items-center gap-2 px-4 py-2 bg-primary text-white text-sm rounded-lg transition-colors ${uploadingBnsp ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer hover:bg-primary/90'}`}>
                                        {uploadingBnsp ? (
                                            <>
                                                <ButtonLoading />
