...
interface RundownTemplate {
    ...
    name: string;
    description?: string;
    items: Omit<RundownItem, 'clientId'>[];
}

interface EditRundownTemplateModalProps {
    open: boolean;
...
export const EditRundownTemplateModal = ({
    open,
    onOpenChange,
    template,
    onSuccess,
}: EditRundownTemplateModalProps) => {
    ...
    useEffect(() => {
        if (template) {
            ...
            setItems(template.items.map(createRundownItem));
        }
    }, [template]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await api.put(`/admin/rundown-templates/${template.id}`, {
                name,
                description,
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto z-[9999]">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        <Icon icon="solar:pen-line-duotone" className="text-primary" height={24} />
                        Edit Rundown Template
                    </DialogTitle>
                </DialogHeader>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                            {items.map((item, index) => (
                                <div key={item.clientId} className="flex gap-2 items-start p-3 border border-ld rounded-md">
