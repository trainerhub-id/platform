...
export const LessonModal: React.FC<LessonModalProps> = ({
  ...
  useEffect(() => {
    if (open) {
      ...
      setUploadType(
        initialData?.videoType === 'mux'
          ? 'mux'
          : initialData?.videoType === 'google-drive'
            ? 'google-drive'
            : 'youtube',
      )
  ...
  const handleMuxUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await api.post(`/kelas/lessons/${initialData.id}/mux-upload`, {
      ...
      const newDuration = assetStatus.duration
        ? formatDuration(assetStatus.duration)
        : formData.duration
      try {
        ...
        alert(
          'Video uploaded but failed to save: ' + (error.response?.data?.message || error.message),
        )
        setUploading(false)
        return
      }
  ...
  const handleYoutubeImport = async () => {
    ...
    try {
      await api.post(`/kelas/lessons/${initialData.id}/mux-from-url`, { url: formData.videoUrl })
  ...
  const handleReplaceVideo = async () => {
    if (
      !window.confirm('Replace video? Video lama akan dihapus dan Anda dapat upload video baru.')
    ) {
      return
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            {hasExistingVideo ? (
              ...
              <Tabs
                value={uploadType}
                onValueChange={(v) => setUploadType(v as 'mux' | 'youtube' | 'google-drive')}
              >
                ...
                <TabsContent value="youtube" className="space-y-3 mt-4">
                  <div className="space-y-2">
                    ...
                    />
                    {!initialData?.id && (
                      <p className="text-xs text-amber-600">⚠️ Simpan lesson terlebih dahulu</p>
                    )}
                ...
                <TabsContent value="google-drive" className="space-y-3 mt-4">
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs text-muted-foreground">
                        <strong>⚠️ Penting:</strong> Video harus di-set sebagai "Anyone with the link
                        can view"
                      </p>
                      <p className="text-xs text-muted-foreground">Format URL yang didukung:</p>
