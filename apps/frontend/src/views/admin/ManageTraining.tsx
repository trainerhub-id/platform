...
const ManageTraining = () => {
    ...
    const [statusFilter, setStatusFilter] = useState<'all' | 'running' | 'upcoming' | 'completed'>('all');
                                        ...
                                        >
                                            <td className="py-4 px-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-12 h-12 rounded-xl bg-gray-100 overflow-hidden shrink-0 shadow-sm border border-gray-100">
                                                        {/* Placeholder Image */}
                                                        <div className="w-full h-full bg-slate-200 flex items-center justify-center text-xs text-gray-500">Img</div>
                                                    </div>
                                                    <div>
                                                        <p className="text-base font-bold text-dark leading-tight">{batch.name}</p>
                                                        <p className="text-xs text-[#B58E36] font-medium mt-1">Trainer: {batch.trainerName}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-4 px-4">
                                                <div className="space-y-1.5">
                                                    <div className="flex items-center gap-1.5 text-sm text-bodytext font-medium">
                                                        ...
                                                        <span>{new Date(batch.startDate).toLocaleDateString()} - {batch.tanggalSelesai ? new Date(batch.tanggalSelesai).toLocaleDateString() : 'TBD'}</span>
                                                    </div>
                                            ...
                                            <td className="py-4 px-4 text-center">
                                                <div className="inline-flex flex-col items-center">
                                                    <span className="text-base font-bold text-dark">{batch.participants}</span>
                                                    {/* Progress bar mocked for now */}
                                                    <span className="text-[10px] text-gray-400 mt-1">{batch.participants}/?</span>
                                                </div>
                                            </td>
                                            <td className="py-4 px-4">
                                                <Badge className={`shadow-none font-semibold px-3 py-1 border-none capitalize
                                                    ${batch.status === 'running' ? 'bg-primary/10 text-primary' :
                                                        batch.status === 'completed' ? 'bg-green-100 text-green-600' :
                                                            batch.status === 'draft' ? 'bg-gray-100 text-gray-600' :
