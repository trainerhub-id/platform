...
export const TodoList: React.FC<Props> = ({ batchId, isAdmin = false }) => {
    ...
    if (loading) {
        return <div className="p-8 text-center text-gray-500"><IconLoader className="animate-spin mx-auto mb-2 text-blue-600" /> Loading tasks...</div>;
    }

    if (error) {
        return <div className="p-4 bg-red-50 text-red-600 rounded-lg">Error loading tasks. Please try again later.</div>;
    }
    ...
    return (
        <div className="py-2">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={refresh} className="p-2 text-gray-500 hover:bg-gray-100 rounded-full transition-colors" title="Refresh">
                    <IconRefresh size={18} />
                </button>
            </div>
