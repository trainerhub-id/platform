...
const Notifications = () => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'warning':
        return 'solar:widget-4-line-duotone'
      case 'info':
        return 'solar:bell-bing-line-duotone'
      case 'success':
        return 'solar:check-circle-line-duotone'
      case 'error':
        return 'solar:danger-circle-line-duotone'
      default:
        return 'solar:bell-bing-line-duotone'
    }
  }
  ...
  const getBgColor = (type: string) => {
    switch (type) {
      case 'warning':
        return 'bg-lightwarning dark:bg-lightwarning'
      case 'info':
        return 'bg-lightinfo dark:bg-lightinfo'
      case 'success':
        return 'bg-lightsuccess dark:bg-lightsuccess'
      case 'error':
        return 'bg-lighterror dark:bg-lighterror'
      default:
        return 'bg-lightprimary dark:bg-lightprimary'
    }
  }
  ...
  return (
    <div className="relative group/menu">
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align="end" className="w-screen sm:w-[360px] py-6 rounded-sm">
          ...
          <SimpleBar className="max-h-80 mt-3">
            {notifications.length === 0 ? (
              <div className="px-6 py-4 text-center text-bodytext">No new notifications</div>
            ) : (
