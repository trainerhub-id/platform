...
type AuthContextValue = {
  ...
  signInEmail: (input: { email: string; password: string; rememberMe?: boolean }) => Promise<{ token?: string | null; user?: RawAuthUser }>;
  signUpEmail: (input: { name: string; email: string; password: string; rememberMe?: boolean }) => Promise<{ token?: string | null; user?: RawAuthUser }>;
...
async function authFetch<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    const error = new Error(message || 'Authentication request failed') as Error & { errors?: Array<{ longMessage: string }> };
...
export function BetterAuthProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const payload = await authFetch<{ user: RawAuthUser; session: RawAuthSession }>('/auth/session', {
        method: 'GET',
      });
  ...
  const signInEmail = useCallback(async (input: { email: string; password: string; rememberMe?: boolean }) => {
    const payload = await authFetch<{ token?: string | null; user?: RawAuthUser }>('/auth/sign-in/email', {
      method: 'POST',
  ...
  const signUpEmail = useCallback(async (input: { name: string; email: string; password: string; rememberMe?: boolean }) => {
    const payload = await authFetch<{ token?: string | null; user?: RawAuthUser }>('/auth/sign-up/email', {
      method: 'POST',
      body: JSON.stringify(input),
    });
    await refresh();
  ...
  const signOut = useCallback(async () => {
    await authFetch<{ success: boolean }>('/auth/sign-out', { method: 'POST' }).catch(() => ({ success: true }));
    setSession(null);
    setUser(null);
  }, []);
...
export function useSignIn() {
  ...
  return {
    ...
    signIn: {
      ...
      attemptSecondFactor: async (..._args: unknown[]) => ({ status: 'complete', createdSessionId: null }),
  ...
  return {
    ...
    signUp: {
      create: async (input: Record<string, unknown>) => {
        ...
        const password = String(input.password ?? '');
        const metadata = (input.unsafeMetadata && typeof input.unsafeMetadata === 'object')
          ? input.unsafeMetadata as Record<string, unknown>
          : {};
        const name = String(input.firstName ?? input.name ?? metadata.firstName ?? email.split('@')[0]);
      ...
      attemptEmailAddressVerification: async (..._args: unknown[]) => ({ status: 'complete', createdSessionId: null }),
    },
  };
}
