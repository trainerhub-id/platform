...
export const CustomizerContextProvider: React.FC<CustomizerContextProps> = ({ children }) => {
  ...
  const [isHoverOpen] = useState<boolean>(true) // Enable hover expansion by default

  // Helper function to determine if sidebar should be open (clicked open OR hover open)
  const getOpenState = () => {
    return isCollapse === 'full-sidebar' || (isHoverOpen && isHover)
  }
