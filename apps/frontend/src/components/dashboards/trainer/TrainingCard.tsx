    ...
    <div className="grid grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-2 md:col-span-4 col-span-12">
        {(() => {
          ...
          return (
            <CardBox className={`p-6 h-full flex flex-col items-center justify-center ${
              isPaid ? 'bg-gradient-to-br from-lightsuccess to-success/5' : 'bg-gradient-to-br from-lightwarning to-warning/5'
            }`}>
              {/* Icon */}
              <div className={`h-16 w-16 rounded-full flex items-center justify-center mb-4 ${
                isPaid ? 'bg-success/20 border-2 border-success/30' : 'bg-warning/20 border-2 border-warning/30'
              }`}>
                <Icon
            ...
            {batch ? (
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-2.5 py-1 rounded-md text-xs font-semibold ${
                        actualStatus === 'running' ? 'bg-success/20 text-success' :
                        actualStatus === 'upcoming' ? 'bg-info/20 text-info' :
                        actualStatus === 'completed' ? 'bg-gray-500/20 text-gray-300' :
                        'bg-warning/20 text-warning'
                      }`}>
                        {actualStatus === 'running' ? 'Sedang Berlangsung' :
                         actualStatus === 'upcoming' ? 'Akan Datang' :
                         actualStatus === 'completed' ? 'Selesai' :
                         'Draft'}
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex flex-col items-center justify-center h-full text-center py-8">
                ...
                <p className="text-sm text-white/60 max-w-md">
                  Anda belum terdaftar dalam batch training. Silakan hubungi admin untuk informasi lebih lanjut.
                </p>
              </div>
            )}
          </div>
