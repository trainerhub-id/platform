    ...
    <div className="py-2">
      ...
      <div className="relative mt-8 mb-4">
        ...
        <div className="flex justify-between relative">
          {journey.map((step) => (
            <div key={step.id} className="flex flex-col items-center z-10" style={{ width: '80px' }}>
              {/* Icon Circle */}
              <div
                className={`h-12 w-12 rounded-full flex items-center justify-center mb-3 ${step.status === "completed"
                  ? ""
                  : step.status === "current"
                    ? "bg-white"
                    : "bg-gray-50"
              ...
              {/* Label */}
              <p
                className={`text-[11px] md:text-xs text-center font-bold whitespace-nowrap ${step.status === "completed"
                  ? ""
                  : step.status === "current"
                    ? "text-[#B8935E]"
                    : "text-gray-500"
