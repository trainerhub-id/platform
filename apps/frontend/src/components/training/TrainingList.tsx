    ...
    return (
        <div className="space-y-4 max-h-[700px] max-w-md overflow-y-auto p-4 custom-scrollbar">
            {trainings.map((training, index) => (
                <CardBox
                    key={training.id}
                    className={`cursor-pointer p-0 overflow-hidden border rounded-2xl ${selectedId === training.id
                ...
                >
                    ...
                    <div className="px-0 relative">
                        ...
                        <div className={`w-full h-32 bg-gradient-to-br from-[#F8F4ED] to-[#EDE5D8] flex items-center justify-center ${training.image ? 'hidden' : ''}`}>
                            <Icon icon="solar:calendar-minimalistic-bold" className="text-[#C4B596]" height={44} />
                        </div>
                    </div>
